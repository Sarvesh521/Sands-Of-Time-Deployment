...
def participant_home(request):
    ...
    if request.method == "POST":
        if not (flag):
            ...
            a = request.POST.get("text_input").strip()
            ...
            if a not in new_arr:
                if a in k_l:
                    ...
                    if type(d[a]) == int:
                        ...
                        return HttpResponse(
                            "Go to 8Bit and get your time increased by "
                            + d[a][1]
                            + "minutes"
                        )
                else:
                    messages.error(request, "Ooops Wrong Code!! Better luck next time ")
            else:
                messages.error(
                    request,
                    "You have already got points for submitting the above code . This is a repeat submission",
                )
    ...
    context = {
        "users": users,
        "user_1": user_1,
        "curr_date_time": str(user_1.sand.month)
        + " "
        + str(user_1.sand.day)
        + ","
        + str(user_1.sand.year)
        + " "
        + str(user_1.sand.hour)
        + ":"
        + str(user_1.sand.minute)
        + ":"
        + str(user_1.sand.second),
        "flag": flag,
    }
